...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.DIN_CSV_COLS AS 
WITH FLATTEN_INICIAL AS (
    SELECT 
        ...
        t.file_name,
        t.file_row_number
    FROM &DB_NAME.BRONZE.CSV_RAW_CONTENT t,
...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.TRANSFROM_IN_CUSTOMER_CB AS
WITH AGGREGATED_INITIAL_CUSTOMER_B AS (
        ...
        FROM &DB_NAME.SILVER.DIN_CSV_COLS
...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.TRANSFROM_IN_CUSTOMER_CA AS
...
INSERT INTO &DB_NAME.SILVER.DIM_CUSTOMERS
SELECT CUSTOMER_ID,
    CUSTOMER_NAME,
    EMAIL,
    SEGMENT,
    ...
    IS_ACTIVE
FROM &DB_NAME.SILVER.TRANSFROM_IN_CUSTOMER_CB 
UNION
SELECT CUSTOMER_ID,
    CUSTOMER_NAME,
    EMAIL,
    ...
    SIGNUP_SOURCE,
    IS_ACTIVE
FROM &DB_NAME.SILVER.TRANSFROM_IN_CUSTOMER_CA
;
...
INSERT INTO &DB_NAME.SILVER.DIM_PRODUCT
...
INSERT INTO &DB_NAME.SILVER.DIM_ORDER
...
---- DIM PAYMENTS ------------------------------------
INSERT INTO &DB_NAME.SILVER.DIM_PAYMENTS
...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.TRANSFORM_TRANSACTION_XML_PARSE AS 
WITH fixed AS (
    ...
    SELECT 
        ...
        FILE_NAME
    FROM &DB_NAME.BRONZE.RAW_XML_INGESTION
    GROUP BY FILE_NAME
),
cleaned AS (
...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.TRANSFORM_TRANSACTION_CLIENT_XML AS 
WITH EXTRACTION_XML AS (
    SELECT 
        ...
        FROM &DB_NAME.SILVER.TRANSFORM_TRANSACTION_XML_PARSE ,
...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.JSON_FIX AS
WITH dedup AS (
    SELECT *
    FROM &DB_NAME.BRONZE.RAW_JSON_INGESTION
    QUALIFY ROW_NUMBER() OVER (
...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.TRANSFORM_TRANSACTION_CLIENT_JSON AS
SELECT 
...
CREATE OR REPLACE VIEW &DB_NAME.SILVER.SLV_TRANSACTIONS AS 
SELECT 
...
FROM (
        SELECT TRANSACTION_ID,
                ...
                PAYMENT_METHOD,
                PAYMENT_AMOUNT
        FROM &DB_NAME.SILVER.TRANSFORM_TRANSACTION_CLIENT_XML
        UNION ALL 
        SELECT TRANSACTION_ID,
                ORDER_ID,
                ORDER_DATE,
                ...
                PAYMENT_METHOD,
                PAYMENT_AMOUNT
        FROM &DB_NAME.SILVER.TRANSFORM_TRANSACTION_CLIENT_JSON
    );
